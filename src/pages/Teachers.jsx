  ...
  return (
    <>
      <Box as="main" m="0 auto" mt="80px" maxWidth="1248px" mb="128px">
        <Box mb="12px">
          <PageTitle>Викладацький_склад</PageTitle>
        </Box>
        <PageDescription>
          Історія кафедри автоматизованих систем управління
        </PageDescription>
        <Box as="ul" display='flex' flexWrap='wrap' gridGap='24px' justifyContent='center'>
          {data.map(({ id, fullname, rank, src }) => (
            <Item key={id}><CardTeacher to={`${id}`} src={src} fullname={fullname} rank={rank} /></Item>
          ))}
        </Box>
          </Box>
          <Footer/>
